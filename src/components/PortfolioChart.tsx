          ...
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
              {data.map((d, i) => (
                <tr key={d.name} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <td className="px-3 py-2 text-right text-gray-700 dark:text-gray-300">
                    ¥{d.marketValue.toLocaleString('ja-JP')}
                  </td>
                  <td className="px-3 py-2 text-right text-gray-600 dark:text-gray-400">
                    {totalMV > 0 ? `${((d.marketValue / totalMV) * 100).toFixed(1)}%` : '—'}
                  </td>
                  <td className="px-3 py-2 text-right text-gray-700 dark:text-gray-300">
                    {d.purchaseAmount > 0 ? `¥${d.purchaseAmount.toLocaleString('ja-JP')}` : '—'}
                  </td>
                  <td className="px-3 py-2 text-right text-green-600 dark:text-green-400">
                    ¥{d.dividend.toLocaleString('ja-JP')}
                  </td>
                  {/* 配当金割合（税引後） */}
                  <td className="px-3 py-2 text-right text-green-700 dark:text-green-300 font-medium">
                    {totalDividendAfterTax > 0
                      ? `${((d.dividendAfterTax / totalDividendAfterTax) * 100).toFixed(1)}%`
                      : '—'}
                  </td>
                  <td className="px-3 py-2 text-right text-gray-700 dark:text-gray-300">
              ...
              <tr className="bg-gray-50 dark:bg-gray-800 font-medium">
                ...
                <td className="px-3 py-2 text-right text-gray-700 dark:text-gray-300">
                  ¥{totalMV.toLocaleString('ja-JP')}
                </td>
                <td className="px-3 py-2 text-right text-gray-600 dark:text-gray-400">100%</td>
                <td className="px-3 py-2 text-right text-gray-700 dark:text-gray-300">
                  {totalPurchase > 0 ? `¥${totalPurchase.toLocaleString('ja-JP')}` : '—'}
                </td>
                <td className="px-3 py-2 text-right text-green-600 dark:text-green-400">
                  ¥{totalDividend.toLocaleString('ja-JP')}
                </td>
